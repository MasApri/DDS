...
class LaporanController extends Controller {
    ...
    public function harian() {
        ...
        return view('laporan.harian', ['halaman' => $halaman, "survei" => $hasil]);
    }
    ...
    public function edit_harian(){
        ...
        $hapus = DB::delete("delete from catatan_harian where id_target = ? and nip = ? and hari_ke = ?", [$id_target, $nip, $hari_ke]);
        
        $affected = DB::insert("INSERT INTO `catatan_harian`(`id_target`, nip, `hari_ke`, `realisasi`) VALUES (?,?,?,?)", [$id_target, $nip, $hari_ke, $realisasi]);
        $affected1 = DB::update("UPDATE `target` SET `realisasi`= (select sum(realisasi) from catatan_harian where nip='$nip' and id_target = $id_target) WHERE survei_id = ? and nip = ?", [$survei_id, $nip]);
